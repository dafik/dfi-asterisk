...
var Linphone = require('./linphone');
var EventEmitter = require('events').EventEmitter;

var nami = require('../../examples/nami-extended/src/nami');
...
function EndpointManager(server) {
    ...
    this.setupEndpoints = function (howMany, transport, technology, context) {
        ...
        technology = technology || 'pjsip';

        if (technology == 'pjsip') {
            createPjsipEndpoints.call(this, _server, transport, howMany, onCreated, this);
 ...
 */
function createPjsipEndpoints(server, transport, howMany, callBack, thisp) {
    ...
    function onPjsipShowEndpoints(err, response) {
        ...
        for (var name in foundEndpoints) {
            if (foundEndpoints.hasOwnProperty(name)) {
                 ...
                 */
                action = new actions.PJSIPShowEndpoint(name);
                waitForEndpoint++;
    ...
    function ready() {
        ...
        for (var i = 0; i < howMany; i++) {
            ...
            linphone = new Linphone({
                ...
                host: server.options.configuration.server.host,
                technology: 'PJSIP'
            });
...
function createSipEndpoints(server, transport, howMany, context, callBack, thisp) {
    ...
    function onSipShowUsers(err, response) {
        ...
        response.getResults().forEach(function (line) {
            match = line.split(/\s+/);
