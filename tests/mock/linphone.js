...
function Linphone(configuration) {
    ...
    function createRandomFileName() {
        ...
        var name = this.configuration.sip;
        this.configuration.files = {};
        this.configuration.files.conf = '/tmp/' + name + '-' + fileName + '.linphone.conf';
        this.configuration.files.std = '/tmp/' + name + '-' + fileName + '.std.log';
        this.configuration.files.err = '/tmp/' + name + '-' + fileName + '.err.log';
         ...
         * @type {string}
         */
        this.emit(Linphone.Events.CONFIG_NAME);
    }

    function startLinphoneProcess(config) {
        this._linphoneProcess = spawn('linphonec', ['-c', this.configuration.files.conf]);
    ...
    function bindEvents() {
        ...
        this.on(Linphone.Events.CONFIG_WRITTEN, crateLogFile.bind(this));
        this.on(Linphone.Events.LOG_CREATED, startLinphoneProcess.bind(this));
        this.on(Linphone.Events.READY, function () {
            this.configuration.files.wav = this.configuration.files.conf.replace('conf', 'wav');
            this._write("soundcard use files");
            this.on(Linphone.Events.SOUNDCARD_CHANGED, function () {
                this._write("record " + this.configuration.files.wav);
            }.bind(this));
    ...
    function writeConfig() {
        ...
        function onFileData(err, data) {
            ...
            fs.writeFile(this.configuration.files.conf, ini.stringify(newData), onFileWritten.bind(this));
        }
        ...
        function onFileWritten(err) {
             ...
             * @type {string}
             */
            this.emit(Linphone.Events.CONFIG_WRITTEN);
        }

    }

    function crateLogFile(filePath) {


        //fs.writeFile(filePath, ini.stringify(newData), onFileWritten.bind(this));
        this.emit(Linphone.Events.LOG_CREATED);
    }
    ...
    function bindLinphoneStdio() {
        ...
        var stdOutListener = newLineStream.call(this, function (message) {
            fs.appendFileSync(this.configuration.files.std, message + "\n");
        ...
        this._linphoneProcess.stderr.on('data', function (data) {
            fs.appendFileSync(this.configuration.files.err, data);
